...
Future<UserStruct?> refreshLogin() async {
  final refreshToken = await actions.readLocalStorage('refresh_token');
  final context = appNavigatorKey.currentContext;
  print("refresh token: " + refreshToken.toString());
  if (refreshToken != null && refreshToken.isNotEmpty) {
    final refreshTokenResponse =
        await AuthenticationGroup.refreshAccessTokenCall.call(
      refreshToken: refreshToken,
    );

    if (refreshTokenResponse?.succeeded ?? false) {
      // Extract the new token and sign in the user
      final newAccessToken = getJsonField(
        refreshTokenResponse?.jsonBody ?? '',
        r'''$.access_token''',
      ).toString();
      ...
      if (newAccessToken.isNotEmpty) {
        ...
        if (user != null) {
          ...
          // Update the current user in FFAppState
          final userStruct = UserStruct.maybeFromMap(
            getJsonField(
              _convertDatesInMap(refreshTokenResponse?.jsonBody) ?? '',
              r'''$.user''',
            ),
          );
          actions.writeLocalStorage(
              'access_token',
              getJsonField(
                _convertDatesInMap(refreshTokenResponse?.jsonBody) ?? '',
                r'''$.access_token''',
          ...
          if (userStruct != null) {
            ...
            if (context != null &&
                (FFAppState().CurrentUser.preferredLocale != null &&
